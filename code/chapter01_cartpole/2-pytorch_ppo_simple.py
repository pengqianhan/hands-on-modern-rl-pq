...
This is 2-pytorch_ppo.py with all SwanLab logging removed; everything else
(structure, flags, prints, the CSV metrics file) is kept the same, so once you
understand this file the full version adds only the SwanLab layer.

How to run:
    # Default: train (no GUI, fast)
    python 2-pytorch_ppo_simple.py

    # Show the GUI demo (pops up the cart animation window after training)
    python 2-pytorch_ppo_simple.py --gui
