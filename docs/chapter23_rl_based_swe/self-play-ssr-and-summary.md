...
假设我们只有一个能够安装依赖并运行测试的开源仓库，没有人工 issue，也没有正确补丁。模型先修改代码，故意让某个新测试失败；随后它只根据测试补丁和当前仓库寻找修复。修好以后，这一轮“注入—修复”就变成了新的训练经验。
...
这一章沿着“任务—奖励—环境—数据扩展”梳理了 RL-based SWE：

- **20.1 节**：SWE-bench 与 RLVR 范式——为什么 SWE 是 RLVR 的理想战场
- **补充阅读**：SWE-RL——从开放软件演化记录中学习开发者解题过程
- **20.2 节**：CWM 从环境轨迹学习软件行为；DeepSWE 在真实环境中扩展纯 RL
- **20.3 节**：SSR 用同一模型交替注入和修复缺陷，减少对人工训练任务的依赖

**核心收获**：

1. 测试让软件任务获得自动反馈，但测试覆盖率决定奖励是否忠实于真实需求。
2. SWE-RL、CWM 与 DeepSWE 使用不同训练对象和配方，不能用一个“简单奖励”口号概括。
3. 显式 world model、value model 和搜索是值得验证的扩展，不能写成论文未公开的既成组件。
4. Self-play 能在没有人工 issue 与测试的情况下产生经验，同时必须过滤无效缺陷并检查真实 issue 迁移。
5. 商业代码 Agent 展示了多步工程能力；是否使用某种 RL 配方，需要公开训练材料支持。
