...
每次补丁都挡住了上一条路径，优化器随后找到另一条高分路径。这就是单点规则的局限：它只能处理已经想到的漏洞。

所谓**分层防御**，就是在任务定义、奖励、策略更新、独立评测和部署权限之间设置彼此独立的检查。某一层没有发现问题时，后面的层仍有机会阻止错误进入真实环境。
...
本节沿着这条轨迹逐步深入。我们先解释模型规模、搜索预算和优化距离为什么会放大奖励误差，再把这种误差落到一套可以执行的训练与发布流程中。

## 25.4.1 为什么优化越强，奖励漏洞越重要
...
## 25.4.2 Reward Model Overoptimization：分数为什么会先升后坏
...
最简单的旧办法是只奖励最终结果：测试通过就给 1 分。它在动作空间受限、验证器覆盖完整时已经足够；代码 Agent 能改动验证器和环境以后，结果分数就不再可靠。
...
训练过程中保存初始、中期、代理奖励最高和独立效用最高的检查点。每个检查点都运行相同的隐藏评测与权限测试。
...
红队首先生成可能绕过规则的轨迹。人工、单元测试或形式化工具确认其中哪些是真失败，再把确认样本同时加入训练集和回归集。
...
单个安全提示只能说明模型在一个条件下怎样回答。配对评测每次只改变一个变量，再测量行为差值。
...
训练与评测都无法证明未来输入上的零失败率。部署层直接控制最坏后果：
...
前四层降低失败进入部署的概率，第五层降低失败发生后的损失。它们解决的是不同问题，因此不能用某一层替代其余层。

## 25.4.4 Alignment Tax：安全提高时，原有能力是否下降
...
## 25.4.5 Inverse Scaling：更大的模型也可能更容易走错路
...
分层防御主要处理能够观察和限制的行为。还有三条研究路线试图改善监督信号本身，或让系统内部过程更容易检查。

### Constitutional AI：把原则变成训练数据
