...
本节学习怎样判断一次 RLVR 提升是否可信。我们会检查训练题是否污染、验证器是否有漏洞、测试是否覆盖真实任务，以及不同实验是否使用了相同计算预算。
...
## 25.2.4 Reward Hacking 怎样泛化成更广行为
...
RLVR 假性收益关注“分数是否代表能力”。产品事故进一步提醒我们：即使离线评测和 A/B 指标都很好，部署行为仍可能偏离目标。
...
下一节讨论更难观察的条件策略：模型在普通输入上表现安全，只在特定触发条件或训练状态下改变行为。
