...
Unlike preference alignment, tool-use reward is often not subjective. It can be designed from objective signals. In fact, this is a direct application of **RLVR (Reinforcement Learning from Verifiable Rewards)** from Chapter 15 to agentic settings.
...
<details>
<summary>Thinking question: What are the similarities and differences between the two-stage paradigm "SFT teaches format + RL teaches strategy" and DPO from Chapter 14?</summary>
...
In standard LLM RL training, such as PPO from Chapter 8 or GRPO from Chapter 15, the training loop is purely GPU-based: the model generates answers on the GPU, the reward model scores them on the GPU, and gradients are computed on the GPU. The slowest part is usually GPU computation.
...
| Reward source                | RM scoring / preference comparison (Chapters 13-14) | Environment execution results (verifiable; Chapter 15 RLVR)           |
| Credit assignment            | Token-level PPO/RLHF (Chapters 7-8)                 | Turn level (across multiple turns, ORM vs PRM)                        |
| GRPO within-group comparison | Comparing multiple answers (Chapter 15)             | Comparing multiple trajectories (same idea applies)                   |
| Experience replay            | DQN replay buffer (Chapter 5)                       | Replay of tool-call trajectories (requires reproducible environments) |
| Policy gradient theorem      | REINFORCE (Chapter 6)                               | Multi-turn policy gradient (turn-level discounting)                   |
| Actor-Critic                 | PPO (Chapter 8)                                     | Agentic PPO (critic estimates turn-level value)                       |
| RLVR                         | Verifiable rewards (Chapter 15)                     | Natural verifiability of tool execution results                       |
...
**Step 3: Train a reward model.** Use preference data to train an RM. This is consistent with the Bradley-Terry model used for Reward Model training in Chapter 13. The key difference is that an agent RM may need independent scores along multiple dimensions, instead of a single scalar score, so that RL training can perform fine-grained credit assignment.
