...
- **维度 — 输入**
  - 纯文本 LLM RL: prompt token
  - VLM RL: 图片 / 视频 / 截图 + prompt token
- **维度 — 关键模块**
  - 纯文本 LLM RL: 语言模型
  - VLM RL: 视觉编码器 + 多模态投影层 + 语言模型
- **维度 — 常见奖励**
  - 纯文本 LLM RL: 答案正确性、偏好分、格式奖励
  - VLM RL: 视觉理解正确性、推理质量、grounding、安全性
- **维度 — 主要风险**
  - 纯文本 LLM RL: reward hacking、模式坍缩
  - VLM RL: 视觉幻觉、跨模态错误归因、视觉编码器退化
- **维度 — 训练策略**
  - 纯文本 LLM RL: PPO、DPO、GRPO
  - VLM RL: GRPO/PPO + 差异化学习率 / 冻结视觉编码器
- **维度 — 评估重点**
  - 纯文本 LLM RL: 答案是否好
  - VLM RL: 是否真的看图、是否基于图像证据推理
...
面对这种情况，更实际的做法是把一次错误拆成几个可观察的检查点：

- **检查点 — 视觉 grounding**
  - 观察什么: 注意力、框选区域、IoU 是否合理
  - 可能的训练动作: 加入 grounding reward 或视觉一致性检查
- **检查点 — 文本推理**
  - 观察什么: 视觉描述对了，但最后推导是否出错
  - 可能的训练动作: 强化推理格式、过程奖励或 verifier
- **检查点 — 跨模态融合**
  - 观察什么: 图像证据和文本结论是否一致
  - 可能的训练动作: 差异化学习率、冻结 ViT、分阶段训练
...
- **环节 — 奖励更新**
  - 主要问题: 是否真正提升长期安全
  - 安全约束: 安全 reward 优先于舒适和效率
- **环节 — 部署监控**
  - 主要问题: 分布外场景是否出现
  - 安全约束: 线上只允许受控更新，不直接试错
...
- **架构 — ViT + Transformer**
  - 视觉编码: 独立 ViT
  - 融合方式: 视觉 token 拼接进语言模型
  - RL 更新范围: 全量或差异化
  - 适用场景: 通用 VLM RL
- **架构 — 统一 Transformer**
  - 视觉编码: 共享 Transformer
  - 融合方式: Patch embedding
  - RL 更新范围: 全量
  - 适用场景: 资源受限场景
- **架构 — 冻结 ViT + 轻量解码器**
  - 视觉编码: 预训练 ViT（冻结）
...
- **指标 — 任务准确率**
  - 异常信号: 停滞或下降
  - 可能的原因与动作: 奖励设计有漏洞，检查奖励分布
- **指标 — 无图对照准确率**
  - 异常信号: 与有图准确率接近
  - 可能的原因与动作: 模型不看图也能得分，加图像敏感度检查
- **指标 — 通用视觉探针成绩**
  - 异常信号: 训练后明显下降
  - 可能的原因与动作: 视觉编码器退化，降低视觉学习率或冻结
- **指标 — 回答长度**
  - 异常信号: 持续膨胀且准确率不涨
  - 可能的原因与动作: 长度套取奖励，加长度惩罚或截断
- **指标 — 格式奖励**
  - 异常信号: 下降
  - 可能的原因与动作: 探索偏离输出模板，强化格式项
- **指标 — IoU / grounding 奖励**
  - 异常信号: 停滞
  - 可能的原因与动作: 定位能力到瓶颈，考虑换数据或工具路线
- **指标 — KL 散度**
  - 异常信号: 飙升
  - 可能的原因与动作: 学习率过大或奖励过尖，参考 23.4 节处理
...
- **挑战 — 奖励归因**
  - 根本原因: 视觉和文本 token 共享一个奖励信号
  - 缓解策略: 差异化学习率 / 过程奖励
  - 权衡: 保护视觉能力 vs 视觉优化
- **挑战 — 视觉幻觉**
  - 根本原因: 模型编造图片中不存在的内容
  - 缓解策略: 视觉 grounding 检查 / 不确定性惩罚 / 反事实对照
  - 权衡: 训练复杂度 vs 可靠性
- **挑战 — 视觉捷径**
  - 根本原因: 不看图靠语言先验也能拿奖励
  - 缓解策略: 图像敏感度检查 / 工具轨迹奖励
...
- **挑战 — 推理延迟**
  ...
  - 权衡: 精度 vs 速度
- **挑战 — 视觉 token 成本**
  - 根本原因: 高分辨率输入 token 数大
  - 缓解策略: 限制分辨率 / token 池化压缩
  - 权衡: 感知精度 vs 训练吞吐
