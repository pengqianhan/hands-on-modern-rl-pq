...
先看一个很朴素的技巧：乐观初始化。Mountain Car 的回报通常是负数，失败 200 步就是 -200，较好的轨迹也往往还是负回报。于是我们可以把没试过的动作先看得"比现实更有希望"一些，比如从 `0` 或 `-5` 开始，而不是一开始就把它们看得很差。这一步的含义是：让 agent 愿意先去试，等真实经验回来后，再把被证伪的动作价值逐渐压低。
