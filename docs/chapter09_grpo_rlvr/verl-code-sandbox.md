...
火山引擎原始教程使用 VKE 集群 + SandboxFusion 云沙箱做大规模分布式训练。本节把这些方案适配到**本地 GPU 环境**：用子进程隔离代替云沙箱，用单卡/多卡脚本代替集群部署，保留相同的算法逻辑和参数配置。完整的工业级代码 Agent 实验放在 [10.5 用 rLLM 训练 DeepCoder Agent](../chapter10_agentic_rl/rllm-deepcoder-lab)，那里更关注 AgentFlow 和 sandbox cookbook；本节更关注如何把代码 verifier 接进 veRL 训练框架。
...
# 用法：veRL 通过 custom_reward_function 配置加载
...
# ==================== 可调参数 ====================
# 模型：Qwen2.5-Coder 是代码专用变体，比通用 Instruct 更适合代码任务
...
本节和 [10.5](../chapter10_agentic_rl/rllm-deepcoder-lab) 讲的是同一个大方向：用 sandbox reward 训练代码模型。区别在于关注点不同：
