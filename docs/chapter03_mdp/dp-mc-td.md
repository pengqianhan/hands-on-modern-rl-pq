...
DP、MC 和 TD 不是三套互不相干的公式。它们都在估计同一个东西：

$$
V^\pi(s)=\mathbb{E}_\pi[G_t\mid S_t=s].
$$

也就是说，从状态 $s$ 出发，继续按照当前策略 $\pi$ 行动，未来平均会得到多少回报。三种方法的区别在于：**为了构造这一次的更新目标，手里到底有哪些信息**。
...
| 公式里的部分 | 它是什么 | DP 为什么能用 |
| ------------ | -------- | -------------- |
| $\pi(a\mid s)$ | 当前策略在状态 $s$ 下选择动作 $a$ 的概率 | 策略已经固定，可以直接读取。 |
| $R(s,a)$ | 做动作 $a$ 后的平均一步奖励 | DP 假设奖励模型已知。 |
| $P(s'\mid s,a)$ | 做动作 $a$ 后到达每个 $s'$ 的概率 | DP 假设转移模型已知。 |
| $V_k(s')$ | 旧表里下一状态的长期价值估计 | 价值表一直保存在算法内部。 |

::: info DP：知道模型，所以直接算期望
DP 保留了奖励模型 $R$ 和转移模型 $P$。因此它不用真的走一条轨迹，而是把所有可能动作、所有可能下一状态都按概率加权，直接算出“平均情况下应该写入多少”。
:::

现在把 DP 的关键前提拿掉：**不知道环境模型**。这句话具体意味着两件事。

第一，不知道 $R(s,a)$，所以不能在公式里直接查“这个动作平均奖励是多少”。智能体只能在实际走完一步后看到这一次的奖励 $r$。

第二，不知道 $P(s'\mid s,a)$，所以不能对所有可能的下一状态做

$$
\sum_{s'}P(s'\mid s,a)V(s')
$$

这样的模型平均。智能体只能看到这一次实际到了哪个 $s'$，以及后面实际发生了什么。

这时 DP 公式里的“按模型求期望”就不能直接用，只能靠采样来逼近。MC 做的替换是：

$$
\boxed{
\text{不知道 } R,P
\quad\Longrightarrow\quad
\text{不再算模型期望，而是等一整条真实轨迹}
}
$$
...
::: info MC：不知道模型，所以用完整经历替代模型平均
MC 不知道 $R$，就用实际观察到的奖励序列；不知道 $P$，就不枚举所有下一状态，而是用策略实际采样出来的整条轨迹。多采样很多条轨迹后，这些 $G_t$ 的平均会逼近 DP 里想要的期望。
:::

MC 还保留了一个条件：它要等 $G_t$ 完整出现。也就是说，它虽然不需要模型，但仍然需要从当前访问一直看到 episode 结束。TD 再拿掉这个等待条件。
...
其中 $r_{t+1}$ 是刚走一步就能看到的真实奖励，$G_{t+1}$ 是从下一状态开始直到终点的完整未来。问题在于：如果不等 episode 结束，$G_{t+1}$ 还不知道。
...
::: info TD：不等完整回报，所以用下一状态价值自举
TD 已经像 MC 一样不需要 $P$ 和 $R$，因为 $r_{t+1}$ 和 $S_{t+1}$ 都来自真实交互。它又比 MC 更早更新，因为它不等后面所有奖励出现，而是用旧表里的 $V(S_{t+1})$ 近似剩下的未来。
:::

把这条推导链放在一起，就是：

| 方法 | 保留了什么 | 去掉了什么 | 所以目标变成 |
| ---- | ---------- | ---------- | ------------ |
| DP | 已知 $R(s,a)$、$P(s'\mid s,a)$，能枚举动作和下一状态 | 没有去掉模型前提 | $\sum_a\pi(a\mid s)[R(s,a)+\gamma\sum_{s'}P(s'\mid s,a)V_k(s')]$ |
| MC | 保留真实交互，保留完整回报 $G_t$ | 不知道 $R,P$，不能做模型平均 | $G_t$ |
| TD | 保留真实一步奖励 $r$ 和真实下一状态 $s'$ | 不知道 $R,P$，也不等待完整 $G_t$ | $r+\gamma V(s')$ |

所以这三者的转换不是换了学习目标，而是一步步减少可用信息：
...
\text{DP 精确求期望}
\quad
\xrightarrow{\text{不知道 }P,R}
\quad
\text{MC 用完整轨迹采样}
\quad
\xrightarrow{\text{不等完整 }G_t}
\quad
\text{TD 用一步自举}
$$

从信息来源看，DP 使用的是“已知规则”，MC 使用的是“完整经历”，TD 使用的是“眼前一步和下一状态估计”。这条变化线很重要：随着我们对环境知道得越来越少、也越来越不愿等待完整未来，算法逐渐从精确求期望转向采样和自举。
...
可以用一个最小程序把这件事验证一遍。下面的代码使用同一个走廊、同一套策略：在 $S$ 和 $M$ 都以 0.8 的概率右走、0.2 的概率左走。DP 用模型直接迭代；MC 用完整轨迹回报做样本平均；TD 每走一步用 $r+V(s')$ 更新。三种方法的更新方式不同，但估计的是同一个 $V^\pi$。
...
def dp_policy_evaluation(n_iter=100):
...
def td_zero(n_episodes=1_000_000, seed=1):
    random.seed(seed)
    V = {s: 0.0 for s in STATES}
    N = {s: 0 for s in STATES}
    for _ in range(n_episodes):
...
for name, values in [
    ("DP", dp_policy_evaluation()),
    ("MC", mc_every_visit()),
    ("TD", td_zero()),
]:
    print(f"{name}: S={values['S']:.3f}, M={values['M']:.3f}, G={values['G']:.3f}")
```

一次运行结果如下：

```text
DP: S=-3.375, M=-1.875, G=0.000
MC: S=-3.374, M=-1.874, G=0.000
TD: S=-3.376, M=-1.874, G=0.000
```

DP 的输出是模型迭代后的确定结果；MC 和 TD 的输出来自采样，所以会有很小波动。采样次数足够多时，它们都会靠近同一组策略价值。这就是这里说的“殊途同归”：三种算法手里拿到的信息不同，更新节奏不同，但在固定策略下，目标都是同一个 $V^\pi$。
