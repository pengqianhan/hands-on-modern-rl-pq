...
TEXT = {
    "English": {
        ...
        "choose_copy": "Every card uses a real Unity environment capture and opens its training recipe. Select a task, review its goal, then start a run.",
        ...
        "preview_copy": "The task opens with an official environment capture. During training it switches to live Unity frames, then finishes with this run's replay GIF.",
    ...
    "中文": {
        ...
        "runtime": "运行环境",
        "choose": "选择一个游戏",
        "choose_copy": "每张卡片均使用真实 Unity 环境画面，并对应一套训练配方。选择任务、阅读目标，然后启动训练。",
        "understand": "训练前先理解任务",
        "observation": "观察",
        "action": "动作",
        "algorithm": "算法",
        ...
        "log_waiting": "等待训练任务…",
        "preview": "任务预览 / 学习后的策略",
        "preview_copy": "打开任务时显示官方环境实景；训练开始后切换为实时 Unity 画面，结束后显示本次运行的回放 GIF。",
        "download": "下载运行摘要",
        "wait_title": "训练正在运行",
...
def build_demo(space_module: Any):
    ...
    def choose_task(language: str, event: gr.SelectData):
    ...
    def switch_language(language: str, key: str, seed: float):
    ...
    def train_with_ui(key: str, budget: float, learning_rate: float, gamma: float, epsilon: float, seed: float, language: str):
        task = get_task(tasks, key)
        copy = copy_for(language)
        params = {
            "budget": int(budget),
    ...
    with gr.Blocks(title=f"Hands-On Modern RL · {local_value(space['title'], 'English')}") as demo:
        ...
        start.click(train_with_ui, inputs=[selected, budget, learning_rate, gamma, epsilon, seed, language], outputs=[status, metric, curve, preview, artifact, console, wait_state, start], concurrency_limit=1)
